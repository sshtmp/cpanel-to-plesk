...
validate_domain() {
    ...
    if [[ ! "$domain" =~ ^[a-zA-Z0-9]([a-zA-Z0-9._-]{0,251}[a-zA-Z0-9])?$ ]]; then
        echo "ERROR: Invalid or unsafe domain name: '$domain'" >&2
        return 1
    fi
    return 0
}
...
if [ "${#domains[@]}" -eq 0 ] ; then
    echo "ERROR: no domains were specified for processing." >&2
    exit 1
fi
...
if [ ! -x "$awstats" ] ; then
    echo "ERROR: $awstats cannot be executed." >&2
    exit 1
fi
...
for domain in "${domains[@]}" ; do
    ...
    if ! validate_domain "$domain"; then
        echo "WARNING: Skipping domain with invalid name: '$domain'" >&2
        continue
    fi
    ...
    if [ -z "$webstat" ] || [ "$webstat" != "awstats" ] ; then
        echo "WARNING: domain $domain does not exist or has non-AWstats statistics engine. Skipping..." >&2
        continue
    fi
    ...
    if [ "$rebuild" -ne 1 ] ; then
        ...
        if [ "$?" -ne 0 ] ; then
            echo "ERROR: failed to merge access_log*. Skipping domain." >&2
            continue
        fi
        ...
        if [ "$has_ssl" == "true" ] ; then
            ...
            if [ "$?" -ne 0 ] ; then
                echo "WARNING: failed to merge access_ssl_log* logs. Skipping SSL statistics rebuild." >&2
                has_ssl="false"
            fi
        fi
    ...
    echo "Successfully rebuilt all html files."
